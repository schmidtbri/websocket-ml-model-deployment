...
clean-build:  ## clean build artifacts
	...
	rm -rf dist
	rm -rf vendors
	rm -rf model_websocket_service.egg-info
...
test-reports: clean-pyc clean-test ## Run unit test suite with reporting
	...
	mkdir ./reports/badge
	python -m coverage run --source model_websocket_service -m pytest --verbose --color=yes --html=./reports/unit_tests/report.html --junitxml=./reports/unit_tests/report.xml $(TEST_PATH)
...
check-codestyle:  ## checks the style of the code against PEP8
	pycodestyle model_websocket_service --max-line-length=120

check-docstyle:  ## checks the style of the docstrings against PEP257
	pydocstyle model_websocket_service

check-security:  ## checks for common security vulnerabilities
	bandit -r model_websocket_service
